...

{
    ...
    for (int i = 0; i < patient_counter; i++)
    {
        if (patients[i].p_id == id && patients[i].status == PATIENT_ACTIVE)
        {
            ...
            printf("| %-5d | %-20s | %-3d | %-6s | %-15s | %-15s | %-15lld |\n", patients[i].p_id, patients[i].p_name, patients[i].p_age, patients[i].p_gender, patients[i].p_disease, patients[i].p_contact_num, patients[i].registration_time);
            printf("-----------------------------------------------------------------------------------\n");
            return;
        }
    }
